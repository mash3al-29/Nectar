...
fun SeeAllScreen(
    ...
    LaunchedEffect(sectionType) {
        viewModel.loadProductsBySection(sectionType)
    }

    when (val state = currentState) {
        is SeeAllState.Loading -> {
            LoadingState()
        }
        is SeeAllState.Error -> {
            ErrorState(error = state.message)
        }
        is SeeAllState.Success -> {
            Scaffold(
                topBar = {
                    CenterAlignedTopAppBar(
                        title = {
                            Text(
                                text = stringResource(sectionType.titleResId),
                ...
                ProductsGrid(
                    ...
                    onAddToCartClick = { product ->
                        viewModel.onEvent(SeeAllEvent.OnAddToCartClicked(product))
                    },
